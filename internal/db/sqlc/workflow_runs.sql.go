...
const listRunNodeJobsOutboxByRun = `-- name: ListRunNodeJobsOutboxByRun :many
SELECT
    id,
    run_id,
    workflow_version_id,
...
func (q *Queries) ListRunNodeJobsOutboxByRun(ctx context.Context, runID uuid.UUID) ([]RunNodeJobsOutbox, error) {
	rows, err := q.db.Query(ctx, listRunNodeJobsOutboxByRun, runID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []RunNodeJobsOutbox{}
	for rows.Next() {
		var i RunNodeJobsOutbox
		if err := rows.Scan(
			&i.ID,
			&i.RunID,
			&i.WorkflowVersionID,
